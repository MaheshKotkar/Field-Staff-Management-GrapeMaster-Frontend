...
import RejectedVisitEditDialog from '../../components/visits/RejectedVisitEditDialog';
...
const VisitsList = () => {
    ...
    const [editVisit, setEditVisit] = useState(null); // the visit currently being edited/resubmitted
    ...
    // If we routed here from NotificationBell with an editVisitId, open that visit's edit dialog
    useEffect(() => {
        if (!loading && visits.length > 0 && location.state?.editVisitId) {
            const targetVisit = visits.find(v => v._id === location.state.editVisitId);
            if (targetVisit && targetVisit.status === 'rejected') {
                setEditVisit(targetVisit);
                // Clear state so it doesn't reopen if they refresh
                window.history.replaceState({}, document.title)
            }
        }
    }, [loading, visits, location.state]);
                            ...
                            <AnimatedCard key={visit._id} delay={index * 0.05}>
                                <CardContent sx={{ p: 0 }}>
                                    ...
                                    {visit.status === 'rejected' && (
                                        <Box sx={{ mt: 2, p: 2, bgcolor: 'error.50', borderRadius: 2, border: '1px solid', borderColor: 'error.100' }}>
                                            <Typography variant="subtitle2" color="error.main" fontWeight={700} display="flex" alignItems="center" gap={1} mb={1}>
                                                <X size={16} /> Rejected — Reason: {visit.rejectionReason || 'No specific reason provided'}
                                            </Typography>
